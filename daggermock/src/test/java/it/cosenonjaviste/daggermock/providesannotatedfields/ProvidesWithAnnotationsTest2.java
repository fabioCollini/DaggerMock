...
public class ProvidesWithAnnotationsTest2 {
    @Rule public final DaggerMockRule<MyComponent> rule = new DaggerMockRule<>(MyComponent.class, new MyModule())
            .provides(MyService.class, "service1", new Provider<MyService>() {
                @Override
                public MyService get() {
                    return new MyServiceImpl("impl1");
                }
            })
            .provides(MyService.class, "service2", new Provider<MyService>() {
                @Override
                public MyService get() {
                    return new MyServiceImpl("impl2");
                }
            })
            .provides(MyService.class, Qualifier1.class, new Provider<MyService>() {
                @Override
                public MyService get() {
                    return new MyServiceImpl("impl3");
                }
            })
            .provides(MyService.class, Qualifier2.class, new Provider<MyService>() {
                @Override
                public MyService get() {
                    return new MyServiceImpl("impl4");
                }
            });
